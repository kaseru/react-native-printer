...
export type PrinterWidthValue = 58 | 80;
...
export interface PrinterImageOptions {
  ...
  imageWidth?: number;
  imageHeight?: number;
  printerWidthType?: PrinterWidthValue;
  // only ios
  paddingX?: number;
}
...
const buildOpenDrawerCommandBase64 = (pin: number = 0, onTime: number = 37, offTime: number = 80) => {
  const command = [0x1b, 0x70, pin & 0xff, onTime & 0xff, offTime & 0xff];
  return Buffer.from(command).toString('base64');
};

const createUnsupportedIOSUSBError = (printerType: string) => new Error(`${printerType} is not supported on iOS`);

const rejectUnsupportedIOSUSB = <T>(printerType: string): Promise<T> => Promise.reject(createUnsupportedIOSUSBError(printerType));
...
const EscUsbPrinter = {
  ...
  printText: (text: string, opts: PrinterOptions = {}): void => {
    ...
    EscUsbPrinterModule.printRawData(textTo64Buffer(text, opts), (error: Error) => console.warn(error));
  },
  ...
  printBill: (text: string, opts: PrinterOptions = {}): void => {
    ...
    EscUsbPrinterModule.printRawData(billTo64Buffer(text, opts), (error: Error) => console.warn(error));
  },
  ...
  printImage: function (imgUrl: string, opts: PrinterImageOptions = {}) {
    ...
    EscUsbPrinterModule.printImageData(imgUrl, opts?.imageWidth ?? 0, opts?.imageHeight ?? 0, (error: Error) => console.warn(error));
  },
  ...
  printImageBase64: function (Base64: string, opts: PrinterImageOptions = {}) {
    ...
    EscUsbPrinterModule.printImageBase64(Base64, opts?.imageWidth ?? 0, opts?.imageHeight ?? 0, (error: Error) => console.warn(error));
  },
  ...
  printColumnsText: (texts: string[], columnWidth: number[], columnAlignment: ColumnAlignment[], columnStyle: string[], opts: PrinterOptions = {}): void => {
    ...
    EscUsbPrinterModule.printRawData(textTo64Buffer(result, opts), (error: Error) => console.warn(error));
  },
  ...
  openDrawer: (pin: number = 0, onTime: number = 37, offTime: number = 80): void => {
    ...
    EscUsbPrinterModule.printRawData(buildOpenDrawerCommandBase64(pin, onTime, offTime), (error: Error) => console.warn(error));
  }
};
...
const EscNetPrinter = {
  ...
  connect: (host: string, port: number, timeout?: number): Promise<IEscNetPrinter> =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        const message = error instanceof Error ? error.message : `Connect to ${host} fail`;
        reject(message);
      }
    }),
  ...
  printText: (text: string, opts = {}): void => {
    if (Platform.OS === 'ios') {
      ...
      EscNetPrinterModule.printRawData(processedText.text, processedText.opts, (error: Error) => console.warn(error));
    } else {
      EscNetPrinterModule.printRawData(textTo64Buffer(text, opts), (error: Error) => console.warn(error));
    }
  },
  ...
  printBill: (text: string, opts: PrinterOptions = {}): void => {
    if (Platform.OS === 'ios') {
      ...
      EscNetPrinterModule.printRawData(billTo64Buffer(text, opts), (error: Error) => console.warn(error));
    }
  },

  printImage: function (imgUrl: string, opts: PrinterImageOptions = {}) {
    if (Platform.OS === 'ios') {
      EscNetPrinterModule.printImageData(imgUrl, opts, (error: Error) => console.warn(error));
    } else {
      EscNetPrinterModule.printImageData(imgUrl, opts?.imageWidth ?? 0, opts?.imageHeight ?? 0, (error: Error) => console.warn(error));
    }
  },

  printImageBase64: function (Base64: string, opts: PrinterImageOptions = {}) {
    if (Platform.OS === 'ios') {
      EscNetPrinterModule.printImageBase64(Base64, opts, (error: Error) => console.warn(error));
    } else {
      EscNetPrinterModule.printImageBase64(Base64, opts?.imageWidth ?? 0, opts?.imageHeight ?? 0, (error: Error) => console.warn(error));
    }
  },
  ...
  printColumnsText: (texts: string[], columnWidth: number[], columnAlignment: ColumnAlignment[], columnStyle: string[] = [], opts: PrinterOptions = {}): void => {
    ...
    if (Platform.OS === 'ios') {
      ...
      EscNetPrinterModule.printRawData(textTo64Buffer(result, opts), (error: Error) => console.warn(error));
    }
  },
  ...
  openDrawer: (pin: number = 0, onTime: number = 37, offTime: number = 80): void => {
    ...
    if (Platform.OS !== 'ios') {
      EscNetPrinterModule.printRawData(buildOpenDrawerCommandBase64(pin, onTime, offTime), (error: Error) => console.warn(error));
    }
  }
};
...
type BluetoothScanCallback = (device: Record<string, unknown> | null, isDone: boolean) => void;

const normalizeBluetoothDevice = (rawDevice: unknown) => {
  const device = typeof rawDevice === 'string' ? JSON.parse(rawDevice) : rawDevice;
...
const TscUsbPrinterModule =
  Platform.OS === 'ios'
    ? {
        connect: (..._args: unknown[]) => rejectUnsupportedIOSUSB('TSC USB printing'),
        disconnect: () => Promise.resolve(),
        printLabel: (..._args: unknown[]) => rejectUnsupportedIOSUSB('TSC USB printing')
      }
    : TscUsbPrinter;
...
BluetoothPrinter.getDeviceList = getDeviceList;
