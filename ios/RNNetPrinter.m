...
@interface RNNetPrinter () <NSStreamDelegate>
...
{
    if (mode != nil && [mode.lowercaseString isEqualToString:@"legacy"]) {
        return @"legacy";
    }
    return @"rawSocket";
}
...
- (BOOL)openSocketToHost:(NSString *)host port:(NSNumber *)port error:(NSError **)error
{
    ...
    if (!readStream || !writeStream) {
        if (error) {
            *error = [NSError errorWithDomain:@"RNNetPrinter" code:500 userInfo:@{NSLocalizedDescriptionKey: @"Cannot create socket streams"}];
        }
    ...
    while ([deadline timeIntervalSinceNow] > 0) {
        ...
        if (status == NSStreamStatusError || status == NSStreamStatusClosed) {
            ...
            if (error) {
                *error = streamError ?: [NSError errorWithDomain:@"RNNetPrinter" code:501 userInfo:@{NSLocalizedDescriptionKey: @"Cannot open printer socket"}];
            }
            [self cleanupStreams];
            return NO;
        }
    ...
    if (error) {
        *error = [NSError errorWithDomain:@"RNNetPrinter" code:408 userInfo:@{NSLocalizedDescriptionKey: @"Printer socket connect timeout"}];
    }
    [self cleanupStreams];
    return NO;
}
...
- (BOOL)writeData:(NSData *)data error:(NSError **)error
{
    if (![self isConnected]) {
        if (error) {
            *error = [NSError errorWithDomain:@"RNNetPrinter" code:503 userInfo:@{NSLocalizedDescriptionKey: @"Printer is not connected"}];
        }
        return NO;
    }
    ...
    while (totalWritten < totalLength) {
        if (self.outputStream.streamStatus != NSStreamStatusOpen) {
            if (error) {
                *error = self.outputStream.streamError ?: [NSError errorWithDomain:@"RNNetPrinter" code:504 userInfo:@{NSLocalizedDescriptionKey: @"Printer output stream is closed"}];
            }
            return NO;
        }
        ...
        if (written < 0) {
            if (error) {
                *error = self.outputStream.streamError ?: [NSError errorWithDomain:@"RNNetPrinter" code:505 userInfo:@{NSLocalizedDescriptionKey: @"Cannot write to printer"}];
            }
            return NO;
        }
        if (written == 0) {
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode
{
    if (eventCode == NSStreamEventErrorOccurred) {
        RCTLogWarn(@"RNNetPrinter stream error: %@", aStream.streamError.localizedDescription);
