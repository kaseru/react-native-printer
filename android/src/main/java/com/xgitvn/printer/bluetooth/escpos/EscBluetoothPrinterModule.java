...
package com.xgitvn.printer.bluetooth.escpos;
...
import com.xgitvn.printer.bluetooth.BluetoothServiceStateObserver;
import com.xgitvn.printer.bluetooth.escpos.command.sdk.Command;
import com.xgitvn.printer.bluetooth.escpos.command.sdk.PrintPicture;
import com.xgitvn.printer.bluetooth.escpos.command.sdk.PrinterCommand;
...
public class EscBluetoothPrinterModule extends ReactContextBaseJavaModule
    ...
    public EscBluetoothPrinterModule(ReactApplicationContext reactContext,
...
//                String text = new String(toPrint, Charset.forName(encoding));
    ...
    public void printPic(String base64encodeStr, @Nullable  ReadableMap options) {
        int width = 0;
        int leftPadding = 0;
        int threshold = 128; // default = 128
        if(options != null){
            width = options.hasKey("width") ? options.getInt("width") : 0;
            leftPadding = options.hasKey("left") ? options.getInt("left") : 0;
            threshold = options.hasKey("threshold") ? options.getInt("threshold") : 128;
        }
        ...
        int nMode = 0;
        if (mBitmap != null) {
            byte[] data = PrintPicture.POS_PrintBMP_WithThreshold(mBitmap, width, nMode, leftPadding, threshold);
